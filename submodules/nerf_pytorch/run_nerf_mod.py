...
def create_nerf(args):
    ...
    input_ch += args.feat_dim - args.feat_dim_appearance  # add another shape dimension

    input_ch_views = 0
    embeddirs_fn = None
    if args.use_viewdirs:
        embeddirs_fn, input_ch_views = get_embedder(args.multires_views, args.i_embed)
    input_ch_views += args.feat_dim_appearance   # views: add another appearance dimension

    output_ch = 5 if args.N_importance > 0 else 4
    skips = [4]
    if 'W_CLIP' in args: # load clipnerf model
        print('CLIP-based model is loaded')
...
#     raw = run_network(pts)
    raw = network_query_fn(pts, viewdirs, network_fn, features)  # The last one is features
