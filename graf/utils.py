...
def to_sphere(u, v):
    theta = 2 * np.pi * u  # xoy平面旋转
    phi = np.arccos(1 - 2 * v)  # z轴向下旋转角度
...
def sample_on_sphere(range_u=(0, 1), range_v=(0, 1)):
    ...
    return to_sphere(u, v)  # 假定camera pose在一个半径为1的球面上采样
...
def look_at(eye, at=np.array([0, 0, 0]), up=np.array([0, 0, 1]), eps=1e-5):
    ...
    z_axis = eye - at  # z_axis: eye - original point, camera 2 world axis
    z_axis /= np.max(np.stack([np.linalg.norm(z_axis, axis=1, keepdims=True), eps]))

    x_axis = np.cross(up, z_axis)  # x axis -> vertical to up and z_axis
    x_axis /= np.max(np.stack([np.linalg.norm(x_axis, axis=1, keepdims=True), eps]))

    y_axis = np.cross(z_axis, x_axis)  # y axis -> vertical to x and y axis
