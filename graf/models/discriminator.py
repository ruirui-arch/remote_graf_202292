...
class Discriminator(nn.Module):
    ...
    def forward(self, input, y=None):  # y是个可有可无的东西
        input = input[:, :self.nc]
        input = input.view(-1, self.imsize, self.imsize, self.nc).permute(0, 3, 1, 2)  # (BxN_samples)xC -> BxCxHxW, an extra reshape process
