...
from submodules.nerf_pytorch.run_nerf_clip import render as clip_render
from functools import partial


class Generator(object):
    def __init__(self, H, W, focal, radius, use_clip, ray_sampler, render_kwargs_train, render_kwargs_test, parameters, named_parameters,
    ...
    def __call__(self, z, y=None, rays=None, clip_feat=None):
        bs = z.shape[0]
        if rays is None:
        ...
        if self.use_clip:
            render_kwargs['clip_features'] = clip_feat
            rgb, disp, acc, extras = clip_render(self.H, self.W, self.focal, chunk=self.chunk, rays=rays,
                                            **render_kwargs)
        else:
            render_kwargs['clip_features'] = None
            rgb, disp, acc, extras = render(self.H, self.W, self.focal, chunk=self.chunk, rays=rays,
                                        **render_kwargs)

        rays_to_output = lambda x: x.view(len(x), -1) * 2 - 1      # (BxN_samples)xC, from [0~1] to [-1~1]?
    ...
    def sample_pose(self):
        ...
        loc = sample_on_sphere(self.range_u, self.range_v)  # [cx, cy, cz], [1, 3]
        ...
        loc = loc * radius  # 应该是单位半球采样再放缩到半径为radius的圆上去
        R = look_at(loc)[0] # [3, 3]应该是一个旋转矩阵

        RT = np.concatenate([R, loc.reshape(3, 1)], axis=1)  # loc.reshape(3, 1): xyz轴的translation
        RT = torch.Tensor(RT.astype(np.float32))  # 我认为是一个3x4的旋转矩阵
        return RT
