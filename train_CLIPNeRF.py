...
from graf.gan_training_clip import CLIPTrainer
...
from submodules.criteria.clip_loss import CLIPLoss
...
if __name__ == '__main__':
    ...
    config = load_config(args.config, 'configs/use_clip.yaml')
    ...
    # z_dist[type]: gauss   z_dist[dim]: 256  z_dist[appearance]: 128
    ...
    if n_test_samples_with_same_shape_code > 0:
        ...
        ptest = ptest.unsqueeze_(1).expand(-1, n_test_samples_with_same_shape_code, -1, -1).flatten(0,
                                                                                                    1)  # (ntest x n_same_shape) x 3 x 4
    ...
    if config['nerf']['finetune_codes']:  # CLIP loss is used
        use_clip = True
    else:
        use_clip = False

    if use_clip:
        print('Stage II, clip finetune the pretrained model')
        clip_loss = CLIPLoss(config['ray_sampler']['N_samples'])
        description = config['data']['desc']
        desc_type = config['data']['desc_type']
        if desc_type == 'text':
            guide_feats = clip_loss.encode_text(description)
        elif desc_type == 'image':
            guide_feats = clip_loss.encode_image(description)  # [1, 512] feats
        else:
            raise NotImplementedError(f'Unknown description type: {desc_type}')

        text_inputs = torch.cat([clip.tokenize(description)]).to(device)

        guide_feats = guide_feats.float()
    ...
    while True:
        ...
        for x_real in train_loader:
            ...
            if use_clip:
                dloss, reg = trainer.discriminator_trainstep_clipLoss(rgbs, y=y, z=z, clip_feat=guide_feats)  # y is a zero vector
            else:
            ...
            if use_clip:
                gloss, clip_loss = trainer.generator_trainstep_withCLIP(y=y, z=z, feats=guide_feats, text_inputs=text_inputs)
                logger.add('losses', 'clip', clip_loss, it=it)
            else:
            ...
            if ((it + 1) % config['training']['print_every']) == 0:
                ...
                if use_clip:
                    clip_loss_last = logger.get_last('losses', 'clip')
                    print('[%s epoch %0d, it %4d, t %0.3f] g_loss = %.4f, clip_loss = %.4f, d_loss = %.4f, reg=%.4f'
                          % (config['expname'], epoch_idx, it + 1, dt, g_loss_last, clip_loss_last, d_loss_last,
                             d_reg_last))
                else:
